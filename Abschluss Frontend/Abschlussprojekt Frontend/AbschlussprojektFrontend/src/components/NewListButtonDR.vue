...
const createList = async () => {
  try {
    ...
    tasklistStore.addTasklist(response.data.id, response.data.label, response.data.description)
    dialog.value = false
    label.value = ''
    description.value = ''
    errorMsg.value = ''
    ...
    if (error.response && error.response.status === 401) {
      errorMsg.value = 'Bitte loggen Sie sich ein um eine neue Liste zu erstellen.'
    } else {
      console.error(error)
    }
  }
